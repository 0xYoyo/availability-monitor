...
describe("generateDateWindows", () => {
  ...
  it("rejects invalid date formats", () => {
    expect(() => generateDateWindows("2026/04/11", "2026-04-13", 2)).toThrow(
      "rangeStart must be in YYYY-MM-DD format"
    );
  });

  it("rejects impossible calendar dates", () => {
    expect(() => generateDateWindows("2026-04-31", "2026-05-03", 2)).toThrow(
      "Invalid calendar date: 2026-04-31"
    );
  });

  it("rejects non-positive night counts", () => {
    expect(() => generateDateWindows("2026-04-11", "2026-04-13", 0)).toThrow(
      "nights must be a positive integer"
    );
  });

  it("rejects inverted ranges", () => {
    expect(() => generateDateWindows("2026-04-14", "2026-04-13", 2)).toThrow(
      "rangeStart must be on or before rangeEnd"
    );
  });
});
